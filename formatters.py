...
def table_format_total(df):
    df = table_format(df)
    df = df.applymap(lambda x: "background-color: silver",
                     subset=slice(len(df.data) - 1, len(df.data) - 1, 1))
    df = df.applymap(lambda x: "font-weight: bold",
                     subset=slice(len(df.data) - 1, len(df.data) - 1, 1))
    return df
...
def table_heatmap(df, cmap='RdBu'):
    ...
    style = style.background_gradient(cmap=cmap, axis=None, vmax=number,
                                      vmin=number * -1)  # .format(":,.0f")
    return style
